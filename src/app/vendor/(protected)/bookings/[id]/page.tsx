...
export default async function VendorBookingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!booking || booking.ground.vendorId !== vendor.vendorId) {
    return (
      <div className="text-center py-20">
        <p className="text-gray-500">Booking not found</p>
        <Link href="/vendor/bookings" className="text-green-600 hover:underline mt-2 inline-block">Back to bookings</Link>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-3xl">
      <Link href="/vendor/bookings" className="text-sm text-gray-500 hover:text-green-700 mb-4 inline-block">&larr; Back to bookings</Link>
      ...
      <Card className="mb-6">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="border-t pt-4 flex items-center justify-between">
            <p className="text-sm text-gray-500">Your commission rate: {(booking.ground.vendor.commissionRate * 100).toFixed(0)}%</p>
            <a
      ...
      {booking.payment && (
        <Card className="mb-6">
          ...
          <CardContent className="space-y-2">
            ...
            <div className="flex justify-between"><span className="text-gray-500">Commission</span><span className="text-yellow-600">-{formatCurrency(booking.payment.commission)}</span></div>
            <div className="flex justify-between font-bold border-t pt-2"><span>Your Net</span><span className="text-green-700">{formatCurrency(booking.payment.vendorNet)}</span></div>
